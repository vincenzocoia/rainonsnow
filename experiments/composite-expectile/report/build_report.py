...
<div class="findings">
  ...
  <div class="finding"><div class="verdict-tag t-mix">Handover, unresolved</div>
    <p><b>The handover weight w = p&#8310; is a poor one.</b> A plain hard graft at the 0.90- or
    0.95-quantile matches or beats it at every return period (0.16 against 0.18 at T = 1000). But
    the hard graft <em>is</em> the step member of the smooth-graft family, and the run swept the
    join over five values while holding the handover weight at one, so it does not compare the two
    constructions &mdash; it compares five members of a family against one other member, and finds
    the one wanting. That w(0.9) = 0.531 gives the tail model majority share at the ten-year level
    says the fix is a sharper handover weight, not a hard join. Deferred to the graft work.</p></div>
...
<p><strong>Do not reuse the fitting weight as the handover weight.</strong> Section 19 shows
w = p&#8310; is a poor handover: a plain hard join at the 0.90-quantile beats it everywhere. It does
not show that hard beats smooth &mdash; the hard graft is the step member of the same family, and
only the join was swept &mdash; so the open question is which handover weight is best, not which
construction. Until that is settled, a hard join at v &isin; [0.90, 0.95] is the safe default
because it is the one setting that has been tuned.</p>
...
def build():
    body = [
        ...
        SPT.replace("FIG_PT", fig(
            "fig-pseudotrue-tail.png",
...
<div class="note"><span class="lab">How flat that ridge is, measured</span>
<p>Flat enough that the "asymptotic target" figures in this report are not converged at the sample
size used to estimate them. Refitting the same criterion to three independent n = 5000 samples
gives &xi;&#770; = 0.149, 0.233 and 0.405 for the one-sided M-quantile of section 15, and 0.213,
0.259 and 0.354 for pure L2 &mdash; a spread of a quarter in the shape parameter, at fifty times
the sample size of the study. The three decimal places these targets are quoted to are therefore
misleading, and comparisons between one estimator's asymptotic target and another's should not be
read at that resolution. The 2000-replicate medians are the reliable summaries throughout, and the
return-level results, which average over the ridge rather than sitting on it, are unaffected. This
is the same near-singular A, seen from a different direction.</p></div>
...
<div class="col">
<p><strong>A hard graft is sufficient, and at v = 0.90 or 0.95 it beats this smooth
graft</strong> at every return period in the table, including the far tail (0.16 against 0.18 at
T = 1000). It is also brittle in the join: at v = 0.50 the same tail fit is 11&times; the
POT&ndash;MLE at the five-year level, worse than not grafting at all, because a hard graft hands
the model <em>all</em> of the probability above the join and a fit tuned for the 1000-year level
has no business describing the five-year one. The smooth graft never does anything like that; its
worst number anywhere is 1.49.</p>

<div class="note"><span class="lab">What this comparison cannot establish</span>
<p>It is tempting to read the table as hard beating smooth, and that reading is not available. Take
the smooth graft's handover weight to the step w(p) = I(p &gt; v) and, with the rescaling folded
in, the smooth construction <em>is</em> the hard graft at v. The hard grafts are members of the
same family, not a rival to it. What the run did was sweep the join over five values while holding
the handover weight fixed at one &mdash; five members of a family against a sixth &mdash; and a
family cannot be shown inferior to its own elements. The honest statement is narrower and still
useful: <b>w = p<sup>6</sup> is a poor handover weight</b>, and the reason is visible below. What a
better one buys is a question for the graft work, not for this report.</p></div>
...
<h3>The practical answer, for now</h3>
<p>Use a hard graft at the 0.90&ndash;0.95 quantile &mdash; not because hard handovers are better,
but because it is the only setting in this family that has been tuned. Everything in this report
that reports a grafted number uses w = p<sup>6</sup> and is therefore reported against a handover
that section 19 has just shown to be poor; the composite estimators' grafted results would improve,
not degrade, under a better one.</p>

<p>The question the graft work should answer is not <em>hard or smooth</em> &mdash; that framing
does not survive the nesting above &mdash; but <em>which handover weight</em>, and whether the
optimum is interior. Three things point at an interior optimum: the hard graft's collapse at
v = 0.50 shows the step is not uniformly good; its brittleness in v shows the criterion is not flat;
