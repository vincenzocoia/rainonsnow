...
the conventional reference discards nine tenths of the sample. Compared directly against a
tail-weighted likelihood, the composite criteria lead when fitting a generalized Pareto tail above
a known threshold and trail when fitting a generalized extreme value distribution to block maxima,
which we read as efficiency recovered on a poorly identified problem rather than as a verdict on
either construction.</p>
</div>
...
<p>Rain-on-snow flooding is a clean instance, and the mixed-process view of it is long
established: Waylen and Woo (1982) modelled annual floods in a Cascade Mountains catchment as the
maximum of a rainfall-generated and a snowmelt-generated series, and fitted the two separately.
Annual maxima at such a catchment are drawn from two populations &mdash; ordinary snowmelt
freshets, moderate and tightly clustered, and rain-on-snow events, rare and heavy &mdash; so the
maximum of the two has a tail governed by the rain-driven mechanism and a body governed by the
snowmelt one. A single GEV cannot describe
...
<p>A fourth response deserves separate mention because it addresses a different failure. The
convergence of normalised maxima to a GEV limit can be extremely slow &mdash; famously so for a
normal parent &mdash; and Fisher and Tippett (1928) observed that approximating instead by a
<em class="term">sequence</em> of extreme-value distributions with shape
...
<div class="col">
<p>with &rho;(u) = |u| recovering the composite quantile estimator, built on the pinball loss of
Koenker and Bassett (1978), and &rho;(u) = u&sup2; giving the composite expectile estimator, built
on the asymmetric least squares of Newey and Powell (1987). Integrating a loss over a range of
levels rather than fitting at one is the device Zou and Yuan (2008) introduced for composite
quantile regression; here the levels are weighted rather than equally spaced. The absolute value on the asymmetry factor is essential and is
...
<div class="col">
<p>Because the family will be misspecified, the object of interest is not a true &theta; but the
pseudo-true limit &mdash; what the estimator converges to with unlimited data. It can be written
down. Let g<sub>p</sub><sup>G</sup> denote the M-quantile identification function (2) of an
arbitrary distribution G, so that the model's level-p functional T<sub>p</sub>(&theta;) is defined
by g<sub>p</sub><sup>F<sub>&theta;</sub></sup>(T<sub>p</sub>(&theta;)) = 0. Differentiating (1) in
&theta; &mdash; the indicator contributes nothing, &rho; being continuous at zero &mdash; the
pseudo-true &theta;* solves</p>
</div>
<div class="eq"><div class="body">&#8747;&#8320;&sup1; w(p) g&#8202;<sub>p</sub><sup>F</sup>(T&#8202;<sub>p</sub>(&theta;)) &nabla;&#8202;<sub>&theta;</sub>T&#8202;<sub>p</sub>(&theta;) dp = 0</div><div class="tag">(4)</div></div>
<div class="col">
<p>with the sensitivity following by implicit differentiation of the defining equation,</p>
</div>
<div class="eq"><div class="body">&nabla;&#8202;<sub>&theta;</sub>T&#8202;<sub>p</sub>(&theta;) = &minus; [&part;g&#8202;<sub>p</sub><sup>F<sub>&theta;</sub></sup>/&part;&theta;] / [&part;g&#8202;<sub>p</sub><sup>F<sub>&theta;</sub></sup>/&part;t]   at t = T&#8202;<sub>p</sub>(&theta;)</div><div class="tag">(5)</div></div>
<div class="col">
<p>The same function g appears twice, at two distributions. At the model it defines the functional
and its sensitivity; at the truth it measures the discrepancy, vanishing exactly when
T<sub>p</sub>(&theta;) is the truth's own level-p M-quantile. So (4) says the pseudo-true parameter
sets a weighted average of the level-by-level discrepancies to zero, weighted by w and by the
sensitivity. The target therefore depends on the weight, and on &psi; only through which functional
is matched. Section 5.5 solves (4) numerically and finds that concentrating w drives &theta;*
towards the limiting GPD, which is the asymptotic justification for tail weighting.</p>

<p>For the pinball loss on GPD(0, &sigma;, &xi;) exceedances the system is elementary. With
T<sub>p</sub> = &sigma;[(1&minus;p)<sup>&minus;&xi;</sup> &minus; 1]/&xi; one has
&part;T/&part;&sigma; = T/&sigma;, and eliminating that equation from the other leaves two explicit
integral equations in (&sigma;, &xi;):</p>
</div>
<div class="eq"><div class="body">&#8747; w(p)[p &minus; F(T&#8202;<sub>p</sub>)] T&#8202;<sub>p</sub> dp = 0,   &#8747; w(p)[p &minus; F(T&#8202;<sub>p</sub>)] (1&minus;p)<sup>&minus;&xi;</sup> log[1/(1&minus;p)] dp = 0</div><div class="tag">(6)</div></div>
<div class="col">
<p>For the other members T<sub>p</sub> has no elementary form, but (5) still delivers
&nabla;<sub>&theta;</sub>T in closed form from the identification equation, so no numerical
differentiation is required anywhere.</p>
</div>

<h3><span class="n">2.4</span>Closed forms</h3>
<div class="col">
...
<div class="eq"><div class="body">&phi;(x) = E[(Y &minus; x)<sup>+</sup>]</div><div class="tag">(7)</div></div>
<div class="col">
...
<div class="eq"><div class="body">k&#8202;&phi;(t) + m &minus; t = 0,    k = (2p &minus; 1)/(1 &minus; p),   m = E[Y]</div><div class="tag">(8)</div></div>
<div class="col">
...
source of a structural problem, visible in (8).</p>
</div>
...
<p>One obvious repair is to substitute the sample mean for the model's in (8). This works
...
<div class="eq" style="margin:14px 0"><div class="body">p&#8202;&phi;(t) = (1 &minus; p)&#8202;[ c + &phi;(t) &minus; &phi;(t &minus; c) ]</div><div class="tag">(9)</div></div>
...
empirical body is used, the construction blends the empirical survival function with the fitted
one, S&#770;(x) = [1 &minus; w(F&#770;<sub>n</sub>(x))]&#8202;[1 &minus; F&#770;<sub>n</sub>(x)] +
w(F&#770;<sub>n</sub>(x))&#8202;S<sub>&theta;&#770;</sub>(x), with the handover weight taken equal to
...
the MLE is efficient &mdash; L-moments beats the MLE at every return period, the small-sample result reported by Hosking and
Wallis (1987), so it is the honest yardstick. Against it, only the one-sided loss wins, and only beyond
...
S6 = """
<h3><span class="n">5.5</span>What the estimators converge to</h3>
<div class="col">
<p>The comparisons above are at a fixed sample size. Solving (4) removes the sampling noise and
isolates what each criterion targets. Fit to exceedances over u = 3.16, the truth's 0.90-quantile,
where threshold stability fixes the asymptotic target: if exceedances over u&#8320; are
GPD(&sigma;&#8320;, &xi;), then over u they are GPD(&sigma;&#8320; + &xi;(u &minus; u&#8320;), &xi;),
so for a GEV(&mu;, &sigma;, &xi;) tail the limiting GPD at u has shape &xi; and scale
&sigma; + &xi;(u &minus; &mu;) &mdash; here (1.6323, 0.2000). The truth's conditional survival
matches that to a maximum log-ratio of 2.7&times;10<sup>&minus;7</sup> at u = 100.</p>
</div>
<div class="tablewrap">
<table>
<caption><b>Table 4.</b> Pseudo-true GPD limits above u = 3.16 with w(p) = p<sup>6</sup>, solved
from (4) to a residual below 10<sup>&minus;10</sup>. The likelihood limit minimises
Kullback&ndash;Leibler divergence and the L-moment limit matches the first two L-moments; both are
obtained exactly. The right-hand block sweeps the weight for the asymmetric square.</caption>
<thead><tr><th>estimator</th><th>scale</th><th>shape</th><th>&nbsp;&nbsp;w = p<sup>m</sup></th><th>scale</th><th>shape</th></tr></thead>
<tbody>
<tr><td>POT maximum likelihood</td><td class="lose">1.1933</td><td class="lose">0.3459</td><td>m = 0</td><td>1.313</td><td>0.2637</td></tr>
<tr><td>POT L-moments</td><td class="lose">1.2011</td><td class="lose">0.3247</td><td>m = 1</td><td>1.346</td><td>0.2520</td></tr>
<tr><td>composite, pinball</td><td>1.3804</td><td>0.2548</td><td>m = 2</td><td>1.367</td><td>0.2448</td></tr>
<tr><td>composite, asymmetric square</td><td>1.4100</td><td>0.2316</td><td>m = 6</td><td>1.410</td><td>0.2316</td></tr>
<tr><td>&alpha;-elastile, &alpha; = 0.5</td><td>1.4090</td><td>0.2326</td><td>m = 10</td><td>1.430</td><td>0.2259</td></tr>
<tr><td>one-sided, k = 4</td><td class="win">1.4176</td><td class="win">0.2291</td><td>m = 20</td><td>1.455</td><td>0.2195</td></tr>
<tr class="ref"><td>limiting GPD (the target)</td><td>1.6323</td><td>0.2000</td><td>&rarr; limit</td><td>1.632</td><td>0.2000</td></tr>
</tbody></table>
</div>
<div class="col">
<p>Three readings. Maximum likelihood and L-moments do <em>not</em> share a limit, 0.3459 against
0.3247: under correct specification they coincide, but under misspecification a
Kullback&ndash;Leibler projection and a moment match are different functionals. &psi; separates the
losses only weakly &mdash; the asymmetric square, elastile and one-sided limits fall within 0.003
of each other, while the pinball sits 0.023 away, so the division that matters is bounded against
unbounded &psi;, not the choice among the unbounded ones. And the weight moves the target
monotonically towards the limiting GPD, which is the asymptotic content of tail weighting: what
...
contamination adds at the threshold, so
S<sub>cond</sub>/S<sub>limit</sub> &rarr; 1/[S(u)(1 + &xi;u/&sigma;)<sup>1/&xi;</sup>] = 0.8629
here. The limit is a target for the shape; the truth is the target for the level.</p></div>
</div>


<h3><span class="n">5.6</span>Against a weighted likelihood</h3>
<div class="col">
<p>The closest existing method is Fung's (2022) maximum weighted likelihood estimator, which tilts
the likelihood rather than the loss. For a model density h and a non-decreasing weight
&omega; bounded away from zero, MWLE maximises</p>
</div>
<div class="eq"><div class="body">&#8467;(&theta;) = &Sigma;<sub>i</sub>&#8202;&omega;(y<sub>i</sub>)&#8202;log h(y<sub>i</sub>; &theta;) &minus; [&Sigma;<sub>i</sub>&#8202;&omega;(y<sub>i</sub>)]&#8202;log C(&theta;),    C(&theta;) = &#8747; h(u; &theta;)&#8202;&omega;(u)&#8202;du</div><div class="tag">(10)</div></div>
<div class="col">
<p>The normaliser makes (10) a genuine log-likelihood for the tilted density h&omega;/C, so under
misspecification it converges to a Kullback&ndash;Leibler projection taken in the tilted metric
&mdash; a tail-weighted version of the same target maximum likelihood aims at. It is therefore a
direct competitor rather than a related idea, and a comparison is owed.</p>

<p>The two weights live on different axes: &omega; acts on the observed value, w on the probability
level. We bridge them through the quantile transform, setting
&omega;(y) = &epsilon; + (1 &minus; &epsilon;)&#8202;w(F&#770;<sub>n</sub>(y)) with
&epsilon; = 10<sup>&minus;3</sup> for Fung's positivity requirement. The normaliser is then exact
rather than quadrature: with &omega; constant between consecutive order statistics,
C(&theta;) = &Sigma;<sub>j</sub> &omega;<sub>j</sub>&#8202;[H(y<sub>(j+1)</sub>; &theta;) &minus;
H(y<sub>(j)</sub>; &theta;)]. Setting w &equiv; 1 recovers the ordinary maximum likelihood estimator
to five decimal places in both families.</p>
</div>
<div class="tablewrap">
<table>
<caption><b>Table 5.</b> GEV family, n = 100, 2000 replicates, MSE relative to the GEV maximum
likelihood estimator. Left block, the contaminated truth of Section 5.1; right block, exactly
correct GEV(0, 1, 0.2). The last column is the median fitted shape under contamination against a
true &xi; = 0.2.</caption>
<thead><tr><th rowspan="2">estimator</th><th colspan="3">contaminated</th><th colspan="3">correct</th><th rowspan="2">&xi;&#770;</th></tr>
<tr><th>T=2</th><th>T=48</th><th>T=1000</th><th>T=2</th><th>T=48</th><th>T=1000</th></tr></thead>
<tbody>
<tr class="ref"><td>GEV maximum likelihood</td><td>1.00</td><td>1.00</td><td>1.00</td><td>1.00</td><td>1.00</td><td>1.00</td><td class="lose">0.054</td></tr>
<tr class="ref"><td>GEV L-moments</td><td>1.24</td><td>1.30</td><td>1.11</td><td>1.05</td><td>0.94</td><td>1.03</td><td class="lose">0.095</td></tr>
<tr><td>MWLE, Fung (2022)</td><td class="lose">32.50</td><td class="lose">2.62</td><td class="win">0.86</td><td class="lose">43.14</td><td>1.25</td><td>1.05</td><td class="win">0.224</td></tr>
<tr><td>composite, asymmetric square</td><td class="lose">4.83</td><td class="lose">1.71</td><td>1.04</td><td class="lose">2.03</td><td>1.30</td><td class="lose">1.32</td><td>0.189</td></tr>
<tr><td>one-sided, k = 4</td><td class="lose">6.11</td><td class="lose">1.85</td><td class="win">0.89</td><td class="lose">2.47</td><td>1.43</td><td class="lose">1.12</td><td>0.168</td></tr>
</tbody></table>
</div>
<div class="col">
<p>In the GEV setting MWLE wins on both counts we care about. It takes the lowest far-tail error
under contamination, 0.86 against 0.89 for the one-sided loss and 1.04 for the asymmetric square,
and it recovers the shape best by a clear margin. More tellingly, its premium under correct
specification is 4 to 5% beyond T = 48 where the composite estimators pay 12 to 32%: weighting a
likelihood retains much of a likelihood's efficiency, and weighting a loss does not. Its body error
is far worse than anything else here, for the reason the composite estimators' is &mdash; the weight
has discarded the body &mdash; and it is repaired the same way, by grafting.</p>

<p>The peaks-over-threshold setting reverses the conclusion.</p>
</div>
<div class="tablewrap">
<table>
<caption><b>Table 6.</b> GPD family, n = 100. Left block, exceedances over the sample
0.90-quantile of the contaminated truth, three parameters, relative to POT&ndash;MLE(0.90). Right
block, exactly correct GPD(0, 0.5, 0.2) with the threshold known, two parameters, relative to the
GPD MLE. Rows are grafted onto an empirical body except where marked ungrafted; the two reference
rows are not grafted.</caption>
<thead><tr><th rowspan="2">estimator</th><th colspan="3">contaminated, POT</th><th colspan="3">correct GPD</th></tr>
<tr><th>T=2</th><th>T=48</th><th>T=1000</th><th>T=2</th><th>T=48</th><th>T=1000</th></tr></thead>
<tbody>
<tr class="ref"><td>reference (POT&ndash;MLE / GPD MLE)</td><td>1.00</td><td>1.00</td><td>1.00</td><td>1.00</td><td>1.00</td><td>1.00</td></tr>
<tr class="ref"><td>GPD L-moments</td><td>&mdash;</td><td>&mdash;</td><td>&mdash;</td><td class="win">0.95</td><td class="win">0.89</td><td class="win">0.83</td></tr>
<tr><td>MWLE, ungrafted</td><td>&mdash;</td><td>&mdash;</td><td>&mdash;</td><td class="lose">16.34</td><td>1.17</td><td>1.03</td></tr>
<tr><td>MWLE, Fung (2022), grafted</td><td class="win">0.97</td><td class="win">0.90</td><td class="win">0.23</td><td>1.31</td><td>1.04</td><td class="lose">1.41</td></tr>
<tr><td>composite, asymmetric square, grafted</td><td class="win">0.98</td><td class="win">0.86</td><td class="win">0.21</td><td>1.29</td><td class="win">0.87</td><td class="win">0.80</td></tr>
<tr><td>one-sided, k = 4, grafted</td><td class="win">0.98</td><td class="win">0.89</td><td class="win">0.18</td><td>1.29</td><td class="win">0.84</td><td class="win">0.63</td></tr>
</tbody></table>
</div>
<div class="col">
<p>Fitting a two-parameter GPD above a known threshold, MWLE buys nothing: 0.99 to 1.03 from
T = 200 outward, which is the neutral result a tilted likelihood should give when the untilted one
is already efficient on a regular problem. The one-sided loss reaches 0.63 in the same comparison,
and under contamination the composite estimators edge ahead as well. The two studies together
suggest that the tilted likelihood's advantage is efficiency recovered on a hard estimation problem
&mdash; the three-parameter GEV, whose shape is poorly identified &mdash; and that on a regular
problem where no efficiency is available, what remains to be won is the choice of which discrepancy
to minimise, where the composite criteria are better placed. We stress that this is one
data-generating process in each family and offer the reading as a hypothesis.</p>

<div class="note"><p>Grafting behaves differently for the two. It improves every composite fit in
both body and tail, and improves MWLE's body while <em>worsening</em> its tail, 1.03 to 1.41. The
graft rescales the fitted survival by its own value at the join, so it inherits the noise in
S<sub>&theta;&#770;</sub>(u); MWLE's pointwise weight leaves that quantity almost unconstrained,
whereas a composite criterion still integrates over levels below the join with small but non-zero
weight.</p></div>
</div>
...
influence function of Proposition 2 is the most accurate far-tail member of the family, and it has
the distinction of being derived from the obstacle rather than interpolated between two existing
losses.</p>

<p>Four qualifications should travel with that conclusion.</p>
...
<p>Fourth, the closest existing method is ahead of ours in one of the two settings we tried.
Section 5.6 puts Fung's weighted likelihood at the lowest far-tail error on the three-parameter GEV
and at a far smaller premium under correct specification, while the composite criteria lead on the
two-parameter GPD above a known threshold. We take the reading that a tilted likelihood recovers
efficiency where an untilted one leaves some on the table, and that on a regular problem where it
does not, the choice of discrepancy is what remains &mdash; but this rests on one data-generating
process in each family, and settling it needs a sweep over the shape, the sample size and the form
of contamination. Any claim that a tail-weighted loss improves on tail-weighted likelihood should
wait for that.</p>

<p>A further direction is the interaction with penultimate approximation. This paper takes the
extreme-value family as exactly correct in the tail and asks only how to protect that fit from the
body; penultimate theory says the family is never exactly correct at a finite level, and that the
shape best describing the data varies with how far into the tail one looks. A composite criterion
that integrates over a range of levels therefore fits a single &xi; across a region where the
penultimate shape is not constant, which is a source of bias the present framework does not model.
Allowing &xi; to vary with p in a manner consistent with second-order regular variation would
address both problems at once, and would also give a principled reason to weight the far tail more
heavily than the near tail beyond the contamination argument &mdash; the tail model gets better as
one moves out.</p>

<p>A last direction moves outside the M-estimator family altogether. Extremiles (Daouia, Gijbels
and Stupfler, 2019) are L-functionals &mdash; the &tau;-extremile is the expected maximum of
r(&tau;) = log&#8202;&frac12;&#8202;/&#8202;log&#8202;&tau; independent copies &mdash; so the
composite loss does not generate them, but the other half of the construction does transfer:
minimising a weighted distance between the empirical and model extremile functions gives a
tail-weighted probability-weighted-moment estimator, whose natural comparison is Hosking and
Wallis (1987) rather than maximum likelihood. Both extreme-value families have closed-form
extremiles. In our study this estimator is the only one with no cost anywhere: at a short reach it
matches the MLE in the body of a correctly specified GPD and removes 26% of the far-tail error,
beating L-moments throughout, though it does not reach the composite estimators' gains under
contamination. The reach must be an intermediate sequence &mdash; capping it at the sample size
leaves the estimator inconsistent, since the empirical extremile then places most of its weight on
the sample maximum.</p>

<p>Several other directions remain. The exponent of the loss can be varied continuously between one and
...
761&ndash;771.</li>
<li><b>Daouia, A., Gijbels, I. and Stupfler, G.</b> (2019). Extremiles: a new perspective on
asymmetric least squares. <i>Journal of the American Statistical Association</i> <b>114</b>,
1366&ndash;1381.</li>
...
180&ndash;198.</li>
<li><b>Fisher, R. A. and Tippett, L. H. C.</b> (1928). Limiting forms of the frequency
distribution of the largest or smallest member of a sample. <i>Proceedings of the Cambridge
Philosophical Society</i> <b>24</b>, 180&ndash;190.</li>
<li><b>Gomes, M. I. and de Haan, L.</b> (1999). Approximation by penultimate extreme value
distributions. <i>Extremes</i> <b>2</b>, 71&ndash;85.</li>
...
<li><b>Waylen, P. and Woo, M.-K.</b> (1982). Prediction of annual floods generated by mixed
processes. <i>Water Resources Research</i> <b>18</b>, 1283&ndash;1286.</li>
...
<footer>
<p><b>Draft note, not part of the manuscript.</b> Volume and page numbers above were checked
against publisher records; the smooth-graft citation in Section 5.1 remains a placeholder. All numerical results are reproducible from the
