...
Interactive apps live under `apps/*`. Run them from the **repository root** so paths such as `derived/` and `devtools::load_all()` resolve:
...
**Data:** POT peaks, distributional-learning predictions, and fitted models from script 4 — e.g. `derived/era5_land_hourly_alps_peaks.rds`, `derived/era5_land_hourly_alps_dl_predictions.rds`, `derived/era5_land_hourly_alps_dl_rqforest_models.rds`.
